...
1. First we'll create a framebuffer object for rendering the depth map:  
```C++
GLuint depthMapFBO;
glGenFramebuffers(1, &depthMapFBO);
```
2. Next we create a 2D texture that we'll use as the framebuffer's depth buffer:
```C++
...
```
3. With the generated depth texture we can attach it as the framebuffer's depth buffer:
...
```
4. We use cubemap in this case:
Since there are six environment farthest face in the scene and all of them can have some shadow on it. Remember to deal with it later.
```C++
...
__Only model matrix is needed here. We have 6 faces now, do not finish projection and view here.__
```GLSL
#version 330 core
...
__Used for deal with 6 different faces view and projection transformation__
```GLSL
#version 330 core
layout (triangles) in;
...
__Limit the far_plane here, do not consider something too far.__
```GLSL
#version 330 core
in vec4 FragPos;
...
glBindFramebuffer(GL_FRAMEBUFFER, 0);
```
10. The render scene function(__Cube map are all inside face__)
```C++
...
```
11. Just render the cube(same for the cube object and cubemap):
```C++
void renderCube()
{
